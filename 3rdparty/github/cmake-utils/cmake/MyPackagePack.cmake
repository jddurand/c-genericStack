MACRO (MYPACKAGEPACK VENDOR SUMMARY)
  ...
  IF (CMAKE_SOURCE_DIR STREQUAL PROJECT_SOURCE_DIR)
	...
	#
	# Set CPACK_INSTALL_SCRIPT that will be executed right before packaging - we use it to generate CPACK_PRE_BUILD_SCRIPT_PC_PATH
	#
    SET (FIRE_POST_INSTALL_CMAKE_PATH ${CMAKE_BINARY_DIR}/fire_post_installs.cmake)
    SET (CPACK_INSTALL_SCRIPT ${CMAKE_CURRENT_BINARY_DIR}/cpack_install_script_pc.cmake)
	FILE (WRITE  ${CPACK_INSTALL_SCRIPT} "SET (CPACK_PRE_BUILD_SCRIPT_PC_PATH \"${CMAKE_CURRENT_BINARY_DIR}/cpack_pre_build_script_pc_${PROJECT_NAME}.cmake\")\n")
    FILE (APPEND ${CPACK_INSTALL_SCRIPT} "message(STATUS \"Generating \${CPACK_PRE_BUILD_SCRIPT_PC_PATH}\")\n")
    FILE (APPEND ${CPACK_INSTALL_SCRIPT} "FILE (APPEND \${CPACK_PRE_BUILD_SCRIPT_PC_PATH} \"set(CMAKE_SYSTEM_NAME ${CMAKE_SYSTEM_NAME})\\n\")\n")
    FILE (APPEND ${CPACK_INSTALL_SCRIPT} "FILE (APPEND \${CPACK_PRE_BUILD_SCRIPT_PC_PATH} \"set(CMAKE_SIZEOF_VOID_P ${CMAKE_SIZEOF_VOID_P})\\n\")\n")
    FILE (APPEND ${CPACK_INSTALL_SCRIPT} "FILE (APPEND \${CPACK_PRE_BUILD_SCRIPT_PC_PATH} \"include(GNUInstallDirs)\\n\")\n")
	#
	# We use environment variables to propagate CMake options
	#
    FILE (APPEND ${CPACK_INSTALL_SCRIPT} "FILE (APPEND \${CPACK_PRE_BUILD_SCRIPT_PC_PATH} \"set(ENV{CMAKE_INSTALL_PREFIX} \\\"\${CMAKE_INSTALL_PREFIX}/LibraryComponent\\\")\\n\")\n")
    FILE (APPEND ${CPACK_INSTALL_SCRIPT} "FILE (APPEND \${CPACK_PRE_BUILD_SCRIPT_PC_PATH} \"set(ENV{CMAKE_INSTALL_LIBDIR} \\\"\${CMAKE_INSTALL_PREFIX}/LibraryComponent/lib\\\")\\n\")\n")
    FILE (APPEND ${CPACK_INSTALL_SCRIPT} "FILE (APPEND \${CPACK_PRE_BUILD_SCRIPT_PC_PATH} \"cmake_path(SET ENV_CMAKE_MODULE_PATH \\\"\${CMAKE_INSTALL_PREFIX}/LibraryComponent/lib/cmake\\\" NORMALIZE)\\n\")\n")
    FILE (APPEND ${CPACK_INSTALL_SCRIPT} "FILE (APPEND \${CPACK_PRE_BUILD_SCRIPT_PC_PATH} \"message(STATUS \\\"ENV_CMAKE_MODULE_PATH is \\\${ENV_CMAKE_MODULE_PATH}\\\")\\n\")\n")
    FILE (APPEND ${CPACK_INSTALL_SCRIPT} "FILE (APPEND \${CPACK_PRE_BUILD_SCRIPT_PC_PATH} \"set(ENV{CMAKE_MODULE_PATH} \\\"\\\${ENV_CMAKE_MODULE_PATH}\\\")\\n\")\n")
    FILE (APPEND ${CPACK_INSTALL_SCRIPT} "FILE (APPEND \${CPACK_PRE_BUILD_SCRIPT_PC_PATH} \"execute_process(COMMAND \\\"${CMAKE_COMMAND}\\\" -G \\\"${CMAKE_GENERATOR}\\\" \\\"--debug-find\\\" -P \\\"${FIRE_POST_INSTALL_CMAKE_PATH}\\\" WORKING_DIRECTORY \\\$ENV{CMAKE_INSTALL_PREFIX})\\n\")\n")
	#
